...
export default function LoansPage() {
  ...
  const [form, setForm] = useState({ customer_id: "", amount: "", interest_rate: "", term_months: "" } as { customer_id: string; amount: string; interest_rate: string; term_months: string });
  ...
  const loadData = async () => {
    const l = await getLoans() as Loan[];
    const c = await getCustomers() as Customer[];
    setLoans(l);
    setCustomers(c);
  };
          ...
          <div className="bg-white rounded-lg shadow p-6 mb-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" value={form.amount} onChange={(e) => setForm({...form, amount: e.target.value})} className="w-full border rounded-lg px-3 py-2" placeholder="50000" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Interest Rate (%)</label>
                <input type="number" value={form.interest_rate} onChange={(e) => setForm({...form, interest_rate: e.target.value})} className="w-full border rounded-lg px-3 py-2" placeholder="12" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Term (Months)</label>
                <input type="number" value={form.term_months} onChange={(e) => setForm({...form, term_months: e.target.value})} className="w-full border rounded-lg px-3 py-2" placeholder="12" />
              </div>
            </div>
            ...
            <tbody>
              {loans.map((loan) => (
                <tr key={loan.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-4">{loan.interest_rate}%</td>
                  <td className="p-4">{loan.term_months} months</td>
                  <td className="p-4">KSh {parseFloat(loan.monthly_payment).toLocaleString()}</td>
                  <td className="p-4">KSh {parseFloat(loan.total_repayment).toLocaleString()}</td>
                  <td className="p-4">
