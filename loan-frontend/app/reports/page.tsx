...
export default function ReportsPage() {
  ...
  const calculatePAR = () => {
    ...
    setParData(par);
    setShowPar(true); setShowCollection(false); setShowIncome(false); setShowMonthly(false); setShowCashflow(false);
  };
  ...
  const exportLoans = () => exportCSV([
    ["ID","Customer","Amount","Total","Balance","Status","Date"],
    ...loans.map(l => [l.id, l.customer_name, l.amount, l.total_amount, l.balance, l.status, new Date(l.created_at).toLocaleDateString()])
  ], "loans.csv");

  const exportPayments = () => exportCSV([
    ["ID","Loan ID","Amount","Source","Code","Date"],
  ...
  const exportCustomers = () => exportCSV([
    ["ID","Name","Phone","National ID","Date Joined"],
    ...customers.map(c => [c.id, c.name, c.phone, c.national_id||"", new Date(c.created_at).toLocaleDateString()])
  ], "customers.csv");

  const exportMonthly = () => exportCSV([
    ["Month","Loans Disbursed","Amount Disbursed","Payments","Amount Collected","Fees Income"],
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="p-4 md:p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-7 gap-3 mb-6">
          ...
          <button onClick={() => { setShowCollection(true); setShowPar(false); setShowIncome(false); setShowMonthly(false); setShowCashflow(false); }}
          ...
          <button onClick={() => { setShowMonthly(true); setShowPar(false); setShowCollection(false); setShowIncome(false); setShowCashflow(false); }}
            className="bg-teal-600 text-white p-3 rounded-lg hover:bg-teal-700 text-left">
            <p className="font-bold text-sm">📅 Monthly</p>
            <p className="text-xs opacity-80">Breakdown</p>
          </button>
          <button onClick={() => { setShowCashflow(true); setShowPar(false); setShowCollection(false); setShowIncome(false); setShowMonthly(false); }}
            className="bg-blue-700 text-white p-3 rounded-lg hover:bg-blue-800 text-left">
            <p className="font-bold text-sm">💼 Cash Flow</p>
            <p className="text-xs opacity-80">Capital tracker</p>
          </button>
          <button onClick={() => { setShowIncome(true); setShowPar(false); setShowCollection(false); setShowMonthly(false); setShowCashflow(false); }}
        ...
        {/* Cash Flow / Capital Tracker */}
        {showCashflow && (
          <div className="bg-white rounded-lg shadow p-6 mb-6">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-bold text-xl text-blue-700">💼 Capital & Cash Flow Tracker</h3>
              <button onClick={() => setShowCashflow(false)} className="text-gray-400 hover:text-gray-600">✕</button>
            </div>

            {/* Capital Input */}
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
              <label className="block text-sm font-medium text-blue-700 mb-2">Total Capital Invested (KSh)</label>
              <div className="flex gap-3 items-center">
                <input type="number" value={capital}
                  onChange={e => setCapital(parseFloat(e.target.value) || 0)}
                  className="border rounded-lg px-3 py-2 text-sm w-48 font-bold text-lg"
                  placeholder="60000" />
                <p className="text-sm text-blue-600">Your total invested capital (initial + additions)</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                <p className="text-xs text-gray-500 mb-1">Capital Invested</p>
                <p className="text-2xl font-bold text-blue-700">KSh {capital.toLocaleString()}</p>
                <p className="text-xs text-gray-400 mt-1">Your money put in</p>
              </div>
              <div className="bg-red-50 rounded-lg p-4 border border-red-200">
                <p className="text-xs text-gray-500 mb-1">Total Disbursed</p>
                <p className="text-2xl font-bold text-red-600">KSh {totalDisbursed.toLocaleString()}</p>
                <p className="text-xs text-gray-400 mt-1">Cash given to customers</p>
              </div>
            ...
            {/* Warning if overextended */}
            {isOverextended && (
              <div className="bg-red-50 border border-red-300 rounded-lg p-4 mb-6">
                <p className="text-red-700 font-bold">⚠️ Cash Flow Warning!</p>
                <p className="text-red-600 text-sm mt-1">
                  You have disbursed KSh {(totalDisbursed - capital - totalRepaid + totalDisbursed).toLocaleString()} more than your available cash.
                  Wait for more collections before disbursing new loans.
                </p>
              </div>
            )}
            ...
            <div className="bg-gray-50 rounded-lg p-6 mb-6">
              <h4 className="font-bold text-lg mb-4 text-gray-700">📊 Your Net Worth Breakdown</h4>
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center py-2 border-b">
                  <span className="text-gray-600">Interest Still to Collect (active loans)</span>
                  <span className="font-bold text-teal-600">+ KSh {totalInterestExpected.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center py-3 bg-blue-50 rounded-lg px-3">
                  <span className="font-bold text-gray-700 text-lg">Total Net Worth</span>
                  <span className="font-bold text-blue-700 text-xl">KSh {totalAssets.toLocaleString()}</span>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div className="bg-white border rounded-lg p-4">
                ...
                <p className="text-xs text-gray-400">KSh {totalOutstanding.toLocaleString()} outstanding</p>
              </div>
              <div className="bg-white border rounded-lg p-4">
                <p className="text-sm text-gray-500">Fully Paid Loans</p>
                <p className="text-2xl font-bold text-green-600">{paidLoans}</p>
                <p className="text-xs text-gray-400">KSh {totalInterestEarned.toLocaleString()} interest earned</p>
              </div>
              <div className="bg-white border rounded-lg p-4">
                <p className="text-sm text-gray-500">Collection Rate</p>
                <p className="text-2xl font-bold text-teal-600">{collectionRate}%</p>
                <p className="text-xs text-gray-400">Of total disbursed</p>
              </div>
            </div>
          </div>
        )}
              ...
              <table className="w-full text-sm min-w-[600px]">
                <thead className="bg-gray-50">
                  <tr className="text-left text-gray-500 border-b">
                    ...
                    <th className="p-3">Fees Income</th>
                  </tr>
                </thead>
                <tbody>
                  {monthlyData.map(([key, d]) => (
                    <tr key={key} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3 font-bold text-red-600">KSh {d.disbursed.toLocaleString()}</td>
                  ...
                  <tr className="bg-gray-50 font-bold">
                    ...
                    <td className="p-3 text-red-600">KSh {totalDisbursed.toLocaleString()}</td>
        ...
        {showIncome && (
          <div className="bg-white rounded-lg shadow p-6 mb-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-2">
                <button onClick={() => exportCSV([
                  ["ID","Loan ID","Customer","Amount","Type","Code","Date"],
                ...
                ], "company-income.csv")} className="bg-indigo-600 text-white px-3 py-1 rounded text-sm">⬇ Export</button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <p className="text-xs text-gray-400 mt-1">Across {activeLoans} active loans</p>
          </div>
          <div className="bg-white rounded-lg shadow p-4">
