...
const generateSchedule = async (loanId) => {
  ...
  if (!loan.disbursed_at && loan.status !== 'active' && loan.status !== 'paid') return;
...
const run = async () => {
  try {
    // Get ALL active and paid loans
    const loans = await pool.query(
      `SELECT id FROM loans WHERE status IN ('active','paid') ORDER BY id`
    );
    console.log(`Regenerating schedules for ${loans.rows.length} loans...`);
    for (const loan of loans.rows) {
      await generateSchedule(loan.id);
    }

    // Mark overdue
    await pool.query(
      `UPDATE repayment_schedule SET status='overdue' WHERE due_date < NOW() AND status='pending'`
    );
