          ...
          <div className="bg-white rounded-lg shadow p-6 mb-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" value={form.interest_amount ? "KSh " + Number(form.interest_amount).toLocaleString() : ""} readOnly className="w-full border rounded-lg px-3 py-2 bg-gray-50 text-gray-700" placeholder="Auto-filled" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Total Repayment (KSh)</label>
                <input type="text" value={form.total_amount ? "KSh " + Number(form.total_amount).toLocaleString() : ""} readOnly className="w-full border rounded-lg px-3 py-2 bg-gray-50 text-gray-700" placeholder="Auto-filled" />
              </div>
            </div>
            {form.amount && form.term_weeks && !form.interest_amount && (<p className="mt-3 text-sm text-red-500">No pricing rule found for this combination.</p>)}
            <button onClick={handleSubmit} disabled={loading || !form.customer_id || !form.amount || !form.term_weeks || !form.interest_amount} className="mt-4 bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-gray-500 border-b">
                <th className="p-4">Customer</th><th className="p-4">Amount</th><th className="p-4">Term</th>
                <th className="p-4">Interest</th><th className="p-4">Total</th><th className="p-4">Balance</th>
                <th className="p-4">Progress</th><th className="p-4">Status</th>
              </tr>
            </thead>
            <tbody>
              {loans.length === 0 ? (
                <tr><td colSpan={8} className="p-4 text-center text-gray-400">No loans found</td></tr>
              ) : (
                loans.map((loan: any) => {
                  ...
                  return (
                    <tr key={loan.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-4">
                        <span className={"px-2 py-1 rounded-full text-xs font-medium " + (loan.status === "paid" ? "bg-green-100 text-green-700" : loan.status === "active" ? "bg-blue-100 text-blue-700" : loan.status === "rejected" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700")}>
                          {loan.status}
                        </span>
                      </td>
                    </tr>
