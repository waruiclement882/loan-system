...
export default function CustomerProfilePage() {
  ...
  const handleKycUpload = async (e: any, docType: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.error) alert(data.error);
      else { alert("Document uploaded successfully!"); loadKyc(); }
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="p-6 max-w-6xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <button onClick={() => router.push("/customers")} className="text-gray-400 hover:text-gray-600 text-sm">← Back to Customers</button>
        ...
        <div className="flex gap-2 mb-4">
          {[
            ...
            { key: "kyc", label: "KYC Documents (" + kycDocs.length + ")" },
          ].map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={"px-4 py-2 rounded-lg text-sm font-medium " + (activeTab === tab.key ? "bg-blue-600 text-white" : "bg-white text-gray-600 hover:bg-gray-50")}>
              {tab.label}
            </button>
          ))}
        </div>
        ...
        {activeTab === "loans" && (
          <div className="bg-white rounded-lg shadow overflow-hidden">
            {loans.length === 0 ? <p className="p-8 text-center text-gray-400">No loans found</p> : (
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b">
                  <tr className="text-left text-gray-500">
                    <th className="px-6 py-3">ID</th><th className="px-6 py-3">Amount</th>
                    <th className="px-6 py-3">Term</th><th className="px-6 py-3">Total</th>
                    <th className="px-6 py-3">Balance</th><th className="px-6 py-3">Status</th><th className="px-6 py-3">Schedule</th>
                  </tr>
                </thead>
                <tbody>
                  {loans.map((loan: any) => (
        ...
        {activeTab === "payments" && (
          <div className="bg-white rounded-lg shadow overflow-hidden">
            {payments.length === 0 ? <p className="p-8 text-center text-gray-400">No payments found</p> : (
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b">
                  <tr className="text-left text-gray-500">
        ...
        {activeTab === "kyc" && (
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">Upload customer identification documents for verification</p>
            <div className="grid grid-cols-2 gap-4">
              {docTypes.map(doc => {
                const existing = kycDocs.find((d: any) => d.document_type === doc.key);
                return (
                  <div key={doc.key} className="border-2 rounded-lg p-4 hover:border-blue-300">
                    <div className="flex justify-between items-center mb-3">
                      <p className="font-medium">{doc.label}</p>
                      {existing ? (
                        <span className="px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs font-medium">✓ Verified</span>
                      ) : (
                        <span className="px-2 py-1 bg-yellow-100 text-yellow-700 rounded-full text-xs font-medium">⚠ Missing</span>
                      )}
                    </div>
                    {existing ? (
                      <div>
                        <a href={existing.document_url} target="_blank" rel="noreferrer"
                          className="text-blue-600 hover:underline text-sm block mb-2">📄 View Document</a>
                        <p className="text-xs text-gray-400">Uploaded {new Date(existing.uploaded_at || existing.created_at).toLocaleDateString()}</p>
                      </div>
                    ) : (
                      <div>
                        ...
                        <label htmlFor={"kyc-" + doc.key}
                          className={"cursor-pointer px-4 py-2 rounded-lg text-sm inline-block " + (uploading ? "bg-gray-400 text-white" : "bg-blue-600 text-white hover:bg-blue-700")}>
